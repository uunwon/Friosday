...
struct NewsService {
    ...
    func searchNews(query: String, page: Int, itemsPerPage: Int) -> AnyPublisher<NewsResponse, Error> {
        ...
        /*
         1. dataTaskPublisher: 지정된 URL 요청('request')에 대해 데이터 가져오는 Publisher를 반환함
                - 이 Publisher는 네트워크 요청 시작하고, 요청 완료하면 Data와 URLResponse 전달함
         2. map: Publisher 의 출력 값을 반환, 키 경로를 통해 응답에서 data만 추출
         3. decode: Data 를 지정된 타입(NewsResponse)🔥으로 디코딩
         4. Publiser의 이벤트가 지정된 스케줄러(메인 스레드)에서 처리되도록 함
                - UI 업데이트는 메인 스레드에서 수행되어야 하므로, 필수 단계 ✨
         5. eraseToAnyPublisher: 🌟 Publisher 를 AnyPublisher 타입으로 변환
